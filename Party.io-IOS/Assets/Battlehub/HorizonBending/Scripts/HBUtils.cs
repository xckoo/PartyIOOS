...
namespace Battlehub.HorizonBending
{
    ...
    {
        _HB_XZ_YUP_ZFWD, _HB_XZ_YUP_XFWD,// _HB_X_YUP, _HB_Z_YUP,
        _HB_YZ_XUP_ZFWD, _HB_YZ_XUP_YFWD,// _HB_Y_XUP, _HB_Z_XUP,
        _HB_XY_ZUP_YFWD, _HB_XY_ZUP_XFWD,// _HB_X_ZUP, _HB_Y_ZUP,
        _HB_OFF
    }
    ...
    public struct HBSettings
    {
        ...
        public HBSettings(BendingMode bendingMode) : this(bendingMode, Vector3.zero, Vector3.zero, Vector3.zero)
        {

        }

        public HBSettings(BendingMode bendingMode, Vector3 curvature, Vector3 flatten, Vector3 horizonOffset, bool relativeToCamera = false)
        {
            ...
            if (bendingMode == BendingMode._HB_XY_ZUP_YFWD || bendingMode == BendingMode._HB_XY_ZUP_XFWD)
            {
                Mask = new Vector3(1.0f, 1.0f, 0.0f);
                Gradient = new Vector3(1.0f, 0.0f, 0.0f);
                Up = new Vector3(0.0f, 0.0f, 1.0f);
            }
            else if (bendingMode == BendingMode._HB_XZ_YUP_ZFWD || bendingMode == BendingMode._HB_XZ_YUP_XFWD)
            {
            ...
            else if (bendingMode == BendingMode._HB_YZ_XUP_ZFWD || bendingMode == BendingMode._HB_YZ_XUP_YFWD)
            {
    ...
    public static class HBUtils
    {
        ...
        public static void HBCurvature(Vector3 curvature)
        {
            Shader.SetGlobalVector("_Curvature", curvature);
        }

        public static void HBFlatten(Vector3 flatten)
        {
            Shader.SetGlobalVector("_Flatten", flatten);
        }

        public static void HBHorizonOffset(Vector3 horizonOffset, Transform transform = null)
        {
            if (transform == null)
            {
                Shader.SetGlobalVector("HorizonOffset", horizonOffset);
            }
            else
            {
                Shader.SetGlobalVector("HorizonOffset", transform.position + horizonOffset);
            }
        }
        ...
        public static void HorizonBend(HBSettings settings, Transform transform = null)
        {
            ...
            HBHorizonOffset(settings.HorizonOffset, transform);
        }
        ...
        public static void ScreenPointToRays(Camera camera, float stride, float curvatureRadius, HBSettings settings, out Ray[] rays, out float[] maxDistances)
        {
            ...
            //dir.y = 0;
           // Vector3 offset = GetOffset(ray.origin + dir, settings, camera.transform);
            //Debug.Log(offset);
            //dir = ray.direction;
            //dir.x -= offset.x;
            //Debug.Log(offset.x);
        ...
        private static void RayToRays(Transform cameraTransform, Ray ray, float stride, float curvatureRadius, HBSettings settings, out Ray[] rays, out float[] maxDistances)
        {
            ...
            float length = stride;
            float minFlatten = Math.Min(settings.Flatten.x, settings.Flatten.y);
            if (minFlatten <= 0)
            {
                minFlatten = 0;
            }
            int raysCount = Mathf.CeilToInt(Mathf.Max(curvatureRadius - minFlatten, 0.0f) / stride);
            if (minFlatten > 0)
            {
                length = minFlatten;
                raysCount++;
        ...
        private static bool GetNextRayOrigin(Ray ray, float stride, float curvatureRadius, HBSettings settings, Transform camera, out Vector3 nextRayOrigin)
        {
            ...
            /*Strange raycasting error is fixed by this code. 
            First I thoght error is caused by camera near plane, but later found that it is not true*/
            direction = directionNorm * (stride - 0.3f) + ray.origin;
        ...
        {
            Vector3 atPoint = Vector3.Scale(settings.HorizonOffset, settings.Mask) + settings.Gradient * radius;
            return GetOffset(atPoint, settings, null);
        }
        ...
        public static Vector3 GetOffset(Vector3 point, HBSettings settings, Transform camera = null)
        {
            ...
            {
                case BendingMode._HB_XY_ZUP_YFWD:
                    {
                        throw new NotImplementedException();
                        //float d1 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.x - point.x) - settings.Flatten.x);
                        //float d2 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.y - point.y) - settings.Flatten.y);
                        //float offset = (d1 * d1 * settings.Curvature.x + d2 * d2 * settings.Curvature.y);
                        //return new Vector3(d1 * d1 * settings.Curvature.z, 0.0f, offset);

                    }
                case BendingMode._HB_XY_ZUP_XFWD:
                    {
                        throw new NotImplementedException();
                        //float d1 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.x - point.x) - settings.Flatten.x);
                        //float d2 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.y - point.y) - settings.Flatten.y);
                        //float offset = (d1 * d1 * settings.Curvature.x + d2 * d2 * settings.Curvature.y);
                        //return new Vector3(0.0f, d2 * d2 * settings.Curvature.z, offset);
                    }

                case BendingMode._HB_XZ_YUP_ZFWD:
                    {
                        float d2 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.z - point.z) - settings.Flatten.z);
                        float offsetX = d2 * d2 * settings.Curvature.y;

                        float d1 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.x - (point.x + offsetX)) - settings.Flatten.x);
                        float offsetY = d1 * d1 * settings.Curvature.x + d2 * d2 * settings.Curvature.z;
                        return new Vector3(offsetX, offsetY, 0.0f);
                    }
                case BendingMode._HB_XZ_YUP_XFWD:
                    {
                        throw new NotImplementedException();
                        //float d1 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.x - point.x) - settings.Flatten.x);
                        //float d2 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.z - point.z) - settings.Flatten.z);
                        //float offset = (d1 * d1 * settings.Curvature.x + d2 * d2 * settings.Curvature.z);
                        //return new Vector3(0.0f, offset, d2 * d2 * settings.Curvature.y);
                    }
                case BendingMode._HB_YZ_XUP_ZFWD:
                    {
                        throw new NotImplementedException();
                        //float d1 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.y - point.y) - settings.Flatten.y);
                        //float d2 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.z - point.z) - settings.Flatten.z);
                        //float offset = (d1 * d1 * settings.Curvature.y + d2 * d2 * settings.Curvature.z);
                        //return new Vector3(offset, d1 * d1 * settings.Curvature.x, 0.0f);
                    }
                case BendingMode._HB_YZ_XUP_YFWD:
                    {
                        throw new NotImplementedException();
                        //float d1 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.y - point.y) - settings.Flatten.y);
                        //float d2 = Mathf.Max(0, Mathf.Abs(settings.HorizonOffset.z - point.z) - settings.Flatten.z);
                        //float offset = (d1 * d1 * settings.Curvature.y + d2 * d2 * settings.Curvature.z);
                        //return new Vector3(offset, 0.0f, d1 * d1 * settings.Curvature.x);
                    }
