...
namespace Battlehub.HorizonBending
{
    ...
    public class HB : MonoBehaviour
    {
        ...
        public Vector3 Curvature = new Vector3(5.0f, 5.0f, 5.0f);
        public Vector3 Flatten = new Vector3(0.0f, 0.0f, 0.0f);
        public Vector3 HorizonOffset = new Vector3(0.0f, 0.0f, 0.0f);
        ...
        private void OnEnable()
        {
            ...
            if (AttachToCamera)
            {
                ApplyAll(Curvature, Flatten, HorizonOffset);
            }
            else
            {
                ApplyAll(Curvature, Flatten, HorizonOffset, transform);
            }
            #else
            ApplyAll(Curvature, Flatten, HorizonOffset);
            #endif
		...
		public void LateUpdate(){
			...
			//ChangeCurvature (new Vector3(0.002f,0,-0.003f));

		}
		public  static bool deg = true;
		private static Vector3 farkarti=new Vector3(0.2f,0,1),farkeksi=new Vector3(-1f,0,-1);
		public static void ChangeCurvature(Vector3 delta)
		{
			HB hb = HB.Instance;
			if (hb == null)
			{
			...
			if (PlayerPrefs.GetInt ("Oyun_modu") != 5) {
				if (deg) {
					...
					//ApplyCurvature (hb.Curvature + delta * Camera.main.velocity.z / 80);

					if ((hb.Curvature.x + delta.x) > farkarti.x) {
						deg = false;
						farkarti.x = UnityEngine.Random.Range (0, 0.2f);
					}
				} else if (deg == false) {

					ApplyCurvature (hb.Curvature - delta * Camera.main.velocity.z / 80);

					if ((hb.Curvature.x + delta.x) < farkeksi.x) {
						deg = true;
						farkeksi.x = UnityEngine.Random.Range (0, -2f);
					}
				}
			}
			...
			//			if (deg) {
			//				
			//				ApplyCurvature (hb.Curvature + delta*Camera.main.velocity.z);
			//				if((hb.Curvature + delta) > 0.7f) 
			//				deg = false;
			//			}
			//			else if(deg==false) {
			//				ApplyCurvature (hb.Curvature - delta*Camera.main.velocity.z);
        ...
        public static void ChangeFlatten(Vector3 delta)
        {
            HB hb = Instance;
            if (hb == null)
            {
        ...
        public static void ChangeHorizonOffset(Vector3 delta, Transform transform = null)
        {
            HB hb = HB.Instance;
            if (hb == null)
            {
            ...
            ApplyHorizonOffset(hb.HorizonOffset + delta, transform);
        }

        public static void ApplyCurvature(Vector3 curvature)
        {
            HB hb = Instance;
            if (hb == null)
            {
        ...
        public static void ApplyFlatten(Vector3 flatten)
        {
            HB hb = Instance;
            if (hb == null)
            {
        ...
        public static void ApplyHorizonOffset(Vector3 horizonOffset, Transform transform = null)
        {
            HB hb = HB.Instance;
            if (hb == null)
            {
            ...
            hb.HorizonOffset = horizonOffset;
            HBUtils.HBHorizonOffset(horizonOffset, transform);
        }

        public static void ApplyAll(Vector3 curvature, Vector3 flatten, Vector3 horizonOffset, Transform transform = null)
        {
            HB hb = HB.Instance;
            if (hb == null)
            {
        ...
        public static HBSettings GetSettings(bool attachToCameraInEditor)
        {
            ...
            Vector3 horizonOffset;
            if (attachToCameraInEditor)
            {
                horizonOffset = hb.HorizonOffset;
            }
            else
            {
                horizonOffset = hb.HorizonOffset + hb.transform.position;
            }

            return new HBSettings(
                hb.BendingMode,
                hb.Curvature * CURVATURE_FACTOR,
                hb.Flatten,
                horizonOffset,
        ...
        public static void DebugScreenPointsToRay(Camera camera)
        {
            ...
            for (int i = 0; i < rays.Length; ++i)
            {
                ...
                //Debug.DrawRay(originalRay.origin, ray.origin - originalRay.origin, Color.yellow, 10.0f);
        ...
        private Vector3 m_previousCurvature;
        private Vector3 m_previousFlatten;
        private Vector3 m_previousHorizonOffset;
        ...
        private void Start()
        {
            ...
            m_previousHorizonOffset = HorizonOffset;
        ...
        private void Update()
        {
            ...
            if (m_previousHorizonOffset != HorizonOffset || positionChanged)
            {
                if (AttachToCamera)
                {
                    HBUtils.HBHorizonOffset(HorizonOffset);
                }
                else
                {
                    HBUtils.HBHorizonOffset(HorizonOffset, transform);
        ...
        public void Internal_Rollback(bool full)
        {
            ...
            Curvature = new Vector3(5.0f, 5.0f, 5.0f);
            Flatten = Vector3.zero;
            HorizonOffset = Vector3.zero;
        ...
        public void Internal_Apply()
        {
            ...
            HBUtils.HorizonBend(new HBSettings(BendingMode, Curvature * CURVATURE_FACTOR, Flatten, HorizonOffset, attachToCamera), fakeCameraTransform);
            HBUtils.HBMode(BendingMode);
        ...
        private void CreateHBFixBounds()
        {
            RendererGroupsIterate(renderer =>
            {
                ...
                if (fixBounds != null && !fixBounds.Lock)
                {
                    ...
                    fixBounds.HorizonOffset = HorizonOffset;
        ...
        private void OnDrawGizmos()
        {
            ...
            if (AttachToCamera)
            {
                ...
                position = Camera.main.transform.position + Vector3.Scale(HorizonOffset, settings.Mask);
            }
            else
            {
                position = transform.position + Vector3.Scale(HorizonOffset, settings.Mask);
            }
            ...
            //Handles.DrawWireDisc(position, setings.Up, Flatten);
            //if (Flatten > 0.1f)
            //{
